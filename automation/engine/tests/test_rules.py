...
# sheet_month: xác minh engine đọc đúng tab tháng (dò tab theo tên 'KPI Tháng N')
eq(R.sheet_month(KPI_NEW), 8, "KPI_NEW: ô 'Tháng (số)' = 8")
eq(R.sheet_month(KPI_MASTER), None, "file cũ không có ô 'Tháng (số)' → None")
eq(R.sheet_month([["Tháng (số) (tự tính)", "→", "12"]]), 12, "đọc tháng 12 từ ô cấu hình")
...
eq(R.mere_applies(3, 5_000_000), True, "áp: 3 đơn + có doanh thu")
eq(R.mere_applies(9, 5_000_000), True, "áp: đủ đơn bất kể age (ad vừa bật lại, age nhỏ)")
eq(R.mere_applies(2, 5_000_000), False, "chưa áp: <3 đơn")
eq(R.mere_applies(5, 0), False, "chưa áp: chưa có doanh thu")
# gate IELTS Thái: có doanh thu + (đủ tuổi ≥7d HOẶC ≥2 đơn) — 1 trong 2
eq(R.mere_applies(0, 5_000_000, min_orders=2, age=7, min_age_days=7), True, "Thái: đủ tuổi 7d + 0 đơn → áp")
eq(R.mere_applies(1, 5_000_000, min_orders=2, age=3, min_age_days=7), False, "Thái: tuổi 3d & 1 đơn (<2) → chưa áp")
eq(R.mere_applies(2, 5_000_000, min_orders=2, age=3, min_age_days=7), True, "Thái: tuổi 3d nhưng ≥2 đơn → áp")
eq(R.mere_applies(3, 0, min_orders=2, age=30, min_age_days=7), False, "Thái: đủ tuổi nhưng 0 doanh thu → chưa áp")
# gate VSTEP: có doanh thu + (đủ tuổi ≥7d HOẶC ≥3 đơn/7d)
eq(R.mere_applies(0, 5_000_000, min_orders=3, age=8, min_age_days=7), True, "VSTEP: tuổi 8d + 0 đơn → áp (đủ tuổi)")
eq(R.mere_applies(3, 5_000_000, min_orders=3, age=2, min_age_days=7), True, "VSTEP: tuổi 2d nhưng 3 đơn → áp (đủ đơn)")
eq(R.mere_applies(2, 5_000_000, min_orders=3, age=5, min_age_days=7), False, "VSTEP: tuổi 5d & 2 đơn (<3) → chưa áp")
# recommend_mere_band (spec IELTS Thái 50/70/100) — chỉ theo band, không phụ thuộc CPL
eq(R.recommend_mere_band(49)[:5], "SCALE", "band <50 → SCALE")
eq(R.recommend_mere_band(60)[:8], "THEO DÕI", "band 50–70 → THEO DÕI")
eq(R.recommend_mere_band(85)[:11], "ĐỀ XUẤT TẮT", "band 70–100 → ĐỀ XUẤT TẮT")
eq(R.recommend_mere_band(110)[:12], "TẮT bắt buộc", "band ≥100 → TẮT bắt buộc")
eq(R.recommend_mere_band(None), None, "None ME/RE → None")
# merge inbox: CPL đòi TẮT + ME/RE 40% đủ cổng (keep_loss_pct=50) → special_keep, final = SCALE (ME/RE thắng)
_f, _sp = R.merge_final("TẮT · Phiên 1", R.recommend_mere_band(40), 40, True, keep_loss_pct=50)
eq(_sp, True, "inbox: CPL TẮT + ME/RE 40%<50 đủ cổng → special_keep")
eq(_f[:5], "SCALE", "inbox: final = SCALE theo ME/RE (cứu, không tắt)")
# merge inbox: ME/RE ≥100 đủ cổng → final TẮT bắt buộc bất kể CPL tốt
_f, _sp = R.merge_final("SCALE +20%", R.recommend_mere_band(120), 120, True, keep_loss_pct=50)
eq(_f[:12], "TẮT bắt buộc", "inbox: ME/RE ≥100 thắng CPL tốt → TẮT bắt buộc")
...
# ---- ma trận 4×4 CPL × ME/RE (recommend_mere_matrix, spec Quân 2026-07-23) --------------------
# mere_tier: 4 tầng khớp band 50/70/100
eq(R.mere_tier(40), "tot", "tier <50 → tot")
eq(R.mere_tier(60), "tb", "tier 50–70 → tb")
eq(R.mere_tier(85), "yeu", "tier 70–100 → yeu")
eq(R.mere_tier(120), "rat_te", "tier ≥100 → rất tệ (trần lỗ)")
eq(R.mere_tier(None), None, "tier None → None")
# 16 ô: (CPL, ME/RE) → tiền tố đề xuất đúng spec
mtx = R.recommend_mere_matrix
eq(mtx("TỐT", 40)[:14], "SCALE MẠNH +50", "ô1 CPL tốt × ME/RE tốt → scale mạnh")
eq(mtx("TRUNG BÌNH", 40)[:9], "SCALE +20", "ô2 CPL TB × ME/RE tốt → scale +20")
eq(mtx("YẾU", 40)[:8], "GIẢM 20%", "ô3 CPL yếu × ME/RE tốt → giảm & tối ưu")
eq(mtx("RẤT TỆ", 40)[:12], "CÂN NHẮC TẮT", "ô4 CPL rất tệ × ME/RE tốt → cân nhắc tắt ngắn hạn")
eq(mtx("TỐT", 60)[:5], "GIỮ ·", "ô5 CPL tốt × ME/RE TB → giữ")
eq(mtx("TRUNG BÌNH", 60)[:5], "GIỮ ·", "ô6 CPL TB × ME/RE TB → giữ")
eq(mtx("YẾU", 60)[:8], "GIẢM 20%", "ô7 CPL yếu × ME/RE TB → giảm 20%")
eq(mtx("RẤT TỆ", 60)[:8], "GIẢM 20%", "ô8 CPL rất tệ × ME/RE TB → giảm/cân nhắc tắt")
eq(mtx("TỐT", 85)[:8], "GIẢM 20%", "ô9 CPL tốt × ME/RE yếu → giảm & xem lại tệp")
eq(mtx("TRUNG BÌNH", 85)[:5], "TẮT ·", "ô10 CPL TB × ME/RE yếu → tắt")
eq(mtx("YẾU", 85)[:5], "TẮT ·", "ô11 CPL yếu × ME/RE yếu → tắt")
eq(mtx("RẤT TỆ", 85)[:5], "TẮT ·", "ô12 CPL rất tệ × ME/RE yếu → tắt")
eq(mtx("TỐT", 120)[:12], "TẮT bắt buộc", "ô13 ME/RE ≥100 → auto tắt bất kể CPL tốt")
eq(mtx("TRUNG BÌNH", 120)[:12], "TẮT bắt buộc", "ô14 ME/RE ≥100 → auto tắt")
eq(mtx("YẾU", 120)[:12], "TẮT bắt buộc", "ô15 ME/RE ≥100 → auto tắt")
eq(mtx("RẤT TỆ", 120)[:12], "TẮT bắt buộc", "ô16 ME/RE ≥100 + CPL rất tệ → auto tắt")
# ô 4/8: "CÂN NHẮC TẮT"/"GIẢM" KHÔNG bị coi là lệnh tắt (để special_keep/exception nhận diện giữ lại)
eq(R._is_kill_rec(mtx("RẤT TỆ", 40)), False, "ô4 CÂN NHẮC TẮT → không tính là kill (được giữ)")
eq(R._is_kill_rec(mtx("RẤT TỆ", 60)), False, "ô8 GIẢM → không tính là kill (được giữ)")
# merge_matrix: exception = CPL 3 ngày đòi tắt NHƯNG ma trận giữ lại
_f, _ex = R.merge_matrix("XEM XÉT TẮT · 0 lead", "RẤT TỆ", 40, True)
eq(_f[:12], "CÂN NHẮC TẮT", "merge_matrix: final = ô4 theo ma trận")
eq(_ex, True, "merge_matrix: CPL tab đòi tắt + ma trận giữ → ngoại lệ (xin duyệt)")
_f, _ex = R.merge_matrix("TẮT · CPL rất tệ", "RẤT TỆ", 85, True)
eq(_f[:5], "TẮT ·", "merge_matrix: ô12 vẫn tắt (ME/RE yếu, không cứu)")
eq(_ex, False, "merge_matrix: cả 2 cùng tắt → không ngoại lệ")
_f, _ex = R.merge_matrix("SCALE +20%", "TỐT", 40, False)
eq(_f, "SCALE +20%", "merge_matrix: chưa đủ cổng → theo CPL 3 ngày")
eq(_ex, False, "merge_matrix: chưa đủ cổng → không ngoại lệ")
