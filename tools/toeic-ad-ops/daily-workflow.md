...
4. Dán vào file `.env` ở gốc dự án (file này đã được `.gitignore`, **không commit**):
   ...
   ```
5. Kiểm tra: `python3 tools/toeic-ad-ops/notify_telegram.py message "test ad-ops ✅"`
...
BƯỚC 1 — Quyết định:
  cd /Users/hocquannguyen/prep-marketing && python3 tools/toeic-ad-ops/daily_gate.py
...
- "ALERT_MORNING" / "ALERT_FINAL" / "ERROR": gửi đúng nội dung field "message" rồi dừng:
    python3 tools/toeic-ad-ops/notify_telegram.py message "<message>"
- "REPORT":
  (a) Đọc & làm theo .claude/commands/mkt-toeic-adops.md để dựng báo cáo 3 ngày cho CẢ HAI tài khoản
      ...
      spend/budget/adset, dựng tools/toeic-ad-ops/meta_spend.json, chạy
      python3 tools/toeic-ad-ops/adops.py tools/toeic-ad-ops/meta_spend.json reports/toeic-adops-3ngay-<HÔM-NAY>.html,
  ...
  (c) Gửi PDF kèm caption tóm tắt tiếng Việt (cửa sổ ngày, tổng chi/ngày dự kiến vs KPI, số bài
      ...
      python3 tools/toeic-ad-ops/notify_telegram.py document "<đường-dẫn-PDF>" "<caption>"
  (d) Gửi PDF OK → python3 tools/toeic-ad-ops/daily_gate.py --mark-sent
...
python3 tools/toeic-ad-ops/daily_gate.py
python3 tools/toeic-ad-ops/daily_gate.py --date=2026-06-19 --hour=10   # mô phỏng một ngày/giờ

# Chỉ kiểm tra gate cào lead
python3 tools/toeic-ad-ops/check_leads.py --date=2026-06-18
```
...
### Lệnh
```bash
python3 tools/toeic-ad-ops/run_daily.py            # chạy thật theo lịch (gate → report/alert)
python3 tools/toeic-ad-ops/run_daily.py --dry-run  # ép chạy report, tạo PDF, KHÔNG gửi / KHÔNG mark-sent
```
...
1. Copy thư mục `tools/toeic-ad-ops/` lên server n8n (vd `/opt/prep-adops/tools/toeic-ad-ops/`).
...
4. Workflow: **Schedule Trigger** (cron `0 10,14 * * *`) → **Execute Command**:
   `cd /opt/prep-adops && python3 tools/toeic-ad-ops/run_daily.py`
